...
<head>
  ...
  <script>
  	function delCheck(idx) {
  		...
  		if(ans) location.href="<%=request.getContextPath()%>/urlMappingDelete.um?idx="+idx;
  	}
  </script>
</head>
<body>
  ...
  <div class="container">
		...
		<br>
		<div>
			<button type="button" class="btn btn-secondary" onclick="location.href='<%=request.getContextPath() %>/urlMappingInput.um';">정보등록</button>
			<button type="button" class="btn btn-secondary" onclick="location.href='<%=request.getContextPath() %>/URLMapping.url2';">돌아가기</button>
		</div>
		<hr>
		<div>
